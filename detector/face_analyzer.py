...
def _get_deepface():
    """Lazy-load DeepFace in a subprocess to avoid TF/PyTorch CUDA conflicts.
    TensorFlow and PyTorch cannot coexist in the same process with CUDA.
    For now, returns None — face recognition will be handled by a separate service."""
    global _deepface
    if _deepface is None:
        with _deepface_lock:
            if _deepface is None:
                logger.warning(
                    "DeepFace disabled in detector process (TF/PyTorch CUDA conflict). "
                    "Face recognition will be available via the face-analyzer microservice."
                )
    return _deepface
